""" Projeto de site de uma padaria """
...
app.config["SECRE_KEY"] = "MinhaChave10000#"
app.config["SIMPLELOGIN_USERNAME"] = "Giovani"
app.config["SIMPLELOGIN_PASSWORD"] = "Junqueira"
...
class Product(db.Model):
    ...
    def __init__(self, nome: str, descricao: str, ingredientes: str,
                 origem:str, imagem: str) -> None:
        self.nome = nome
...
def listar_produtos():
    if request.method == 'POST':
        ...
        resultado = db.session.execute(db.select(Product).filter(
            Product.nome.like(f'%{termo}%'))).scalars()
...
def editar_produto(id):
    if request.method == 'POST':
        ...
        print(produto_editado.nome)
        return render_template('editar.html', produto=produto_editado)

@app.route('/deletar_produto/<int:id>')
@login_required
def deletar_produto(id):
    produto_deletado = produto = db.session.execute(db.select(Product).filter(Product.id == id)).scalar()
